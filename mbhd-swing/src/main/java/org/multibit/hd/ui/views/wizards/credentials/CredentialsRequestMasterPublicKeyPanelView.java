...
public class CredentialsRequestMasterPublicKeyPanelView extends AbstractHardwareWalletWizardPanelView<CredentialsWizardModel, String> {
  ...
  public void fireInitialStateViewEvents() {
    ...
    // Initialise with "Next" disabled to force users to work with hardware wallet
  ...
  public void afterShow() {
    ...
    // Ensure that the title has updated to match the current wallet mode
    SwingUtilities.invokeLater(new Runnable() {
      @Override
      public void run() {
        final MessageKey operationKey;
        final boolean nextEnabled;
        final boolean createNewHardwareWallet;
        if (!features.isPresent()) {
          operationKey = MessageKey.HARDWARE_FAILURE_OPERATION;
          nextEnabled = true;
          createNewHardwareWallet = false;
        } else {
        ...
        getWizardModel().setSwitchToPassword(nextEnabled && !createNewHardwareWallet);
        getWizardModel().setCreateNewHardwareWallet(createNewHardwareWallet);
